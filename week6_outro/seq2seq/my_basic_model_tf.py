...
class BasicTranslationModel:
    ...
    def symbolic_translate(self, inp, greedy=False, max_len = None, eps = 1e-30, **flags):
        ...
        def step(blob,t):
            h_prev, y_prev = blob[0], blob[2]
            h_new, logits1 = self.decode(h_prev, y_prev, **flags)
            y_new = tf.argmax(logits1,axis=-1) if greedy else tf.multinomial(logits,1)[:,0]
